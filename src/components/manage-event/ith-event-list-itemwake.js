    ...
    return html`
      ...
      <div class="layout horizontal">
       ...
       <div class="flex-1 cell-content border" id="radi">
         <paper-radio-button name="{{item.deviceId}}" on-load='_load' on-click="_change" data-args="{{item.deviceId}}" value="{{item.tag}}" id="[[item.tag]]"></paper-radio-button>
...
// alert(this.email+"sefws")
  ...
  sensorList()
  {
   ...
   eload(dev,gen)
   {
    ...
    if(gen=="true"||gen=="1")
     {
  this.$.wake.checked=true;
     }
   else if(gen=="false"||gen=="0")
     {
  this.$.missed.checked=true;
      }
     else if(gen=="2")
      {
  ...
  //alert(dev);
   //this.subscriber=null;
 /*for(var i=0;i<this.subscriber.devices.length;i++)
   {
    
     for(var j=0;j<dev.length;j++)
       {
        
          if(this.subscriber.devices[i].tag==dev[j])
            {
            // this.shadowRoot.querySelector('#dev[j]').selected;
             this.$['.items'].selected;
