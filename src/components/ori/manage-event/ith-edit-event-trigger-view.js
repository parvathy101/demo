...
class IthEditEventTriggerView extends PolymerElement {
  static get template() {
    return html`
       ...
       <div class="add-template-view-container">
        ...
        <div class="layout vertical trigger-view-row-container flex-8">
          <div class="alert-text">Will trigger event?</div>
           <paper-radio-group>
              <paper-radio-button name="take_palce" id="yes">Yes</paper-radio-button>
              <paper-radio-button name="tale_not_palce" id="no">No</paper-radio-button>
           </paper-radio-group>
          </div>
        </div>
        </div>
...
load(time,timeType,duration,start,end)
{
console.log(time+"llllllll"+timeType);

 //var time = {};
if(timeType=="AtTime")
{
this.$.timeType.value="At Time";
var t= time.split(' ')[1];
this.$.time.value=t;
}
else if(timeType=="TimePeriod")
{
this.$.timeType.value="Time Period";
var st= start.split(' ')[1];
var en= end.split(' ')[1];
this.$.startTime.value=st;
this.$.endTime.value=en;
}
else if(timeType=="WithinPrevious")
{
this.$.timeType.value="Within";
...
window.customElements.define('ith-edit-event-trigger-view', IthEditEventTriggerView);
