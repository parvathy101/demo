...
class TeleHealthcareFlowSearchCaretaker extends PolymerElement {
  static get template() {
    return html`
  ...
  _handleResponse(e) {
    var response = e.detail.responses.caretakers;
    this.dispatchEvent(new CustomEvent("caretaker-result", { detail: { 'caretakers': response }}));
  }

  search(qsearch,mail) {
      this.$.client._dataChanged();
      this._postEvent = "searchcaretaker.php";
      var postData = {};
      postData.searchval = qsearch;
      postData.subscriber = mail;
      
      this.$.client.postSmart(postData);
  }
}

window.customElements.define('telehealthcareflow-searchcaretaker', TeleHealthcareFlowSearchCaretaker);
