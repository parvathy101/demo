...
class TeleHealthcareFlowRemoveDevice extends PolymerElement {
  static get template() {
    return html`
  ...
  _handleResponse(e) {
   ...
   this.dispatchEvent(new CustomEvent("session-expired", { detail: { 'session': response2}}));
     ...
     {
    this.dispatchEvent(new CustomEvent("device-remove", { detail: {  "message" : response }}));
     }
  }

  remove(id,subscriber) {
      this.$.client._dataChanged();
      this._postEvent = "removedevice.php";
      var postData = {};
...
window.customElements.define('telehealthcareflow-removedevice', TeleHealthcareFlowRemoveDevice);
