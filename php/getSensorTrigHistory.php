...
require $_SERVER['DOCUMENT_ROOT'].'/php/objects/SensorTrigEvent.php';
...
)
{
  // echo  json_encode(array("message" => "sensorTag and dateStr available"));
   $event->readEx4($data->subscriber,$data->sensorTag,$data->dateStr,100);
   $event->outAsJSON();
}
else if(
   !empty($data->sensorTag) &&
   empty($data->dateStr)
	   ...
	   $event->readExSensor($data->subscriber,$data->sensorTag,100);
	   $event->outAsJSON();
   }
else if(
    empty($data->sensorTag) &&
